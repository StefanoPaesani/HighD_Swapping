...
@numba.jit(nopython=True)
def threshold_detection_prob_faster(covM, means, det_pattern, hbar=2):
    ...
    m = len(nonzero_idxs)
    p_sum = 1  # empty set is not included in my powerset function so you can start at 1
    for i in powerset(np.arange(m)):
        ia = np.array(i)
        ii = np.concatenate((ia, ia + m), axis=0)
        avec0 = avec_cond[ii]
        Q0 = numba_ix(Qcond, ii, ii)
